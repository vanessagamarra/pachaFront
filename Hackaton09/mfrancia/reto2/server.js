var fs = require("fs"); // llamamos a la librería httpvar http = require("http"); var url = require('url');
...
var miServidor = http.createServer((peticion, respuesta) => { 
    ...
    switch (url.pathname) {
        ...
        case "/ejercicio/14":
            ...
            parrafo_inner = "Una compañía, fabrica focos de colores (verdes, blancos y rojos). Se desea contabilizar, de un lote de N focos, el número de focos de cada color que hay en existencia"
            answ = ejercicio14(3)
            var html = fs.readFileSync(__dirname + "/templates/ejercicio.html") 
                .toString() 
                .replace("%titulo%", titulo_inner) 
...
function ejercicio03(largo,ancho){
    let respuesta = (largo * ancho)
    return respuesta;
}
...
function ejercicio05(radio){
    let respuesta = Math.PI * radio * radio
    return respuesta;
}
...
function ejercicio09(nacimiento,){
    ...
    let respuesta = (anio_actual - nacimiento + " años") ;
    return respuesta;
...
function ejercicio11(aniosEnTienda,){
    ...
    const aniosEnTiendaNumero = parseInt(aniosEnTienda);

    let bonoAsignado = 0;

    // Asignamos el bono según la cantidad de años en la tienda
    if (aniosEnTiendaNumero === 1) {
    bonoAsignado = 100;
    } else if (aniosEnTiendaNumero === 2) {
    bonoAsignado = 200;
    } else if (aniosEnTiendaNumero === 3) {
    bonoAsignado = 300;
    } else if (aniosEnTiendaNumero === 4) {
    bonoAsignado = 400;
    } else if (aniosEnTiendaNumero === 5) {
    bonoAsignado = 500;
    } else {
    bonoAsignado = 1000;
    }
...
function ejercicio12(salario, tasa, años) {
    const salarioInicial = salario;
    const tasaInteres = tasa;
    let salarioActual = salarioInicial;
    let respuesta = '';
  
    for (let i = 1; i <= años; i++) {
      salarioActual = salarioActual * (1 + tasaInteres);
      respuesta += `<li>Salario en el año ${i}: ${salarioActual.toFixed(2)}</li>`; // Agregar el salario actualizado a la cadena
    }
  
    return respuesta; // Retornar la cadena completa con todos los salarios
}
