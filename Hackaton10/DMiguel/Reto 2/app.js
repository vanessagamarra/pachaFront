...
var miServidor = http.createServer((peticion, respuesta) => {
  ...
  switch (peticion.url) {
    ...
    case "/ejercicio/1":
      ...
      generarEjercicio(respuesta, titulo, parrafo, new_inputs, 2, peticion.url);
...
function generarEjercicio(respuesta,titulo,parrafo,new_inputs,cant_inp,domine) {
  new_inputs = repeatText(new_inputs, cant_inp);
  new_inputs = `<form id="form-ejercicio" action="${domine}" method="POST">${new_inputs}<button type="submit" id="ejecutar" class="btn btn-primary">Ejecutar</button></form>`;
  let html = fs
